...
1. Publish or select a valid GitHub release tag such as `v0.3.2`.
...
7. Verify the platform library is present:
   ...
   A Plugin Store install writes a **versioned** library under the platform subdirectory, for example `/CLIProxyAPI/plugins/linux/amd64/account-health-pushover-v0.3.2.so` — not an unversioned file at the plugins root.
